...
echo ${IPMI} ${RAW} ${BUS_NO} ${MUX_ADDR} ${READ_LEN} ${WRITE_DATA}
RET=`${IPMI} ${RAW} ${BUS_NO} ${MUX_ADDR} ${READ_LEN} ${WRITE_DATA} 2>&1`
#echo $RET

READ_LEN=0x01
RET=`${IPMI} ${RAW} ${BUS_NO} ${MUX_ADDR} ${READ_LEN} 2>&1`
if [ $? -eq 0 ];then
    echo "set MUX to:" $RET
else
    echo "set MUX fail"
...
printf "%s\r" "${IPMI} ${RAW} ${BUS_NO} ${M2_ADDRS} ${READ_LEN} ${WRITE_DATA}"
RET=`${IPMI} ${RAW} ${BUS_NO} ${M2_ADDRS} ${READ_LEN} ${WRITE_DATA} 2>&1`
if [ $? -eq 0 ];then
    echo ""
    echo $RET
